...
final class SmallSiteBuildBench
{
    ...
    public function setUp(): void
    {
        ...
        $this->sourceDataDir = __DIR__ . '/data/content';

        if (!is_dir($this->sourceDataDir)) {
            throw new RuntimeException('Benchmark data not found. Run: make bench-generate');
        }

        $this->tempDir = sys_get_temp_dir() . '/yiipress-build-bench-' . uniqid();
        $this->contentDir = $this->tempDir . '/content';
        $this->outputDir = $this->tempDir . '/output';
        $this->touchFile = $this->contentDir . '/collection-1/2034-10-09-entry-3934.md';
    ...
    private function runBuild(string $args): void
    {
        ...
        if ($exitCode !== 0) {
            throw new RuntimeException("Build benchmark command failed:\n" . implode("\n", $output));
        }
    }
