...
- **edit_link** — set to `false` to hide the edit-page action for this entry. When omitted, set to `null`, or set to `true`, the entry inherits the site-wide `edit_page` configuration. The legacy `editLink` key remains supported; `edit_link` wins when both are present. This does not affect the report-issue action
...
- **last_updated** — set to `true` or `false` to override last-updated output for this entry. When omitted or set to `null`, the entry inherits the site-wide `last_updated` setting. Disabled entries do not read the source modification time
