...
- **faq_level** — controls where `::: question` blocks appear. Omit it or set it to `false` to keep each question inline, use `0` to collect all questions at the end of the page, or use a heading level from `1` to `6` to collect questions at the end of each matching heading section
