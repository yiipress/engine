...
The static executable includes `ext-highlighter`, so syntax highlighting does not need FFI or an external shared library. It intentionally omits `ext-openssl`; self-update uses an available host downloader instead. `serve` uses ReactPHP stream sockets, serves built files and live reload SSE in the server loop, keeps one shared live reload watcher per server process, and does not require PHP's native `sockets` extension. On POSIX platforms with PCNTL and signal support, `serve` can spawn multiple worker processes sharing the listening socket; Windows static binaries run a single server process.
