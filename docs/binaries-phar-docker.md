...
The `Run Tests` workflow runs PHPUnit in the Linux Docker test image and also builds `yiipress.exe` on `windows-2022`. The Windows job tests the packaged executable itself by initializing a site, creating a blog entry, building output, checking generated links, and cleaning the output directory.
