...
On Windows, the preview server runs as a single ReactPHP server process. Multiple `--workers` (each spawned as an independent process, sharing the listening socket) and POSIX signal handlers are used only on platforms where PCNTL and signal constants are available.
