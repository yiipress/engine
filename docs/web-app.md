...
The packaged PHAR and static binary keep the same `serve` command, but they do not require a `public/` directory beside the binary. They route requests through the embedded web application and resolve `content/` and `output/` from the current working directory. The static binary uses ReactPHP stream sockets with preforked worker processes for packaged serving, so live reload and other long-lived requests do not block unrelated requests and it does not require PHP's native `sockets` extension.
