...
The static executable includes `ext-yiipress_highlighter`, so syntax highlighting does not need FFI or an external shared library. Packaged `serve` uses ReactPHP stream sockets with preforked worker processes and does not require PHP's native `sockets` extension.
