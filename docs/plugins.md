...
Hooks are configured with `YiiPress\Hook\HookDispatcher`. They are intended for plugin behavior that needs build lifecycle context, final page HTML, or build-wide side effects.

Available events:

- `BuildStartedEvent::NAME` (`build.started`) — fired after site config, navigation, collections, and authors are parsed, before entries and support files are written
- `BuildFinishedEvent::NAME` (`build.finished`) — fired after entries, support files, assets, sitemap, search index, taxonomies, and author pages are generated
- `RenderStartedEvent::NAME` (`render.started`) — fired before an entry or standalone page render starts
- `RenderFinishedEvent::NAME` (`render.finished`) — fired after final page HTML is rendered; listeners may replace the HTML via `setHtml()`

Register listeners through DI:
...
For reusable plugins, implement `HookInterface`:

```php
use YiiPress\Hook\HookEventInterface;
use YiiPress\Hook\HookInterface;

final class BuildStatsHook implements HookInterface
{
    public function handle(HookEventInterface $event): void
    {
        // Inspect the event type and react.
    }
}
```
...
When no listeners are configured, hooks use a fast empty path and do not allocate per-render event objects.
