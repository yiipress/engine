        ...
        <div class="buttons">
            <?php if (count($uiLanguages) > 1): ?>
                <label class="ui-language-control">
                    <span class="sr-only" data-ui-key="ui_language"><?= $h($t('ui_language')) ?></span>
                    <select id="ui-language-selector" class="ui-language-selector" aria-label="<?= $h($t('ui_language')) ?>" data-ui-attr-aria-label="ui_language">
<?php foreach ($uiLanguages as $availableLanguage): ?>
                        <option value="<?= $h($availableLanguage) ?>"<?= $availableLanguage === $uiLanguage ? ' selected' : '' ?>><?= $h($languageName($availableLanguage)) ?></option>
<?php endforeach; ?>
                    </select>
                    <script>
                        (function () {
            ...
            <?php endif; ?>
            <button id="theme-toggle" class="icon-btn" type="button" aria-label="<?= $h($t('toggle_dark_mode')) ?>" data-ui-attr-aria-label="toggle_dark_mode"></button>
        </div>
    </div>
</header>
<?php if ($search): ?>
<div id="search-overlay" hidden></div>
<div id="search-modal" role="dialog" aria-modal="true" aria-label="<?= $h($t('search')) ?>" data-ui-attr-aria-label="search" hidden>
