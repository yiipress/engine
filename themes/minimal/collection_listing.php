...
<?php if ($entries === []): ?>
        <p data-ui-key="no_entries"><?= $h($t('no_entries')) ?></p>
<?php else: ?>
        <ul class="entry-list">
...
<?php if ($entry['dateISO'] !== '' && $entry['dateISO'] > date('Y-m-d')): ?>
                <span class="badge badge-future" data-ui-key="scheduled" data-ui-params="<?= $h((string) json_encode(['date' => $entry['date']], JSON_THROW_ON_ERROR), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>"><?= $h($t('scheduled', ['date' => $entry['date']])) ?></span>
<?php endif; ?>
